import os, textwrap, sys, time, random
from dotenv import load_dotenv
load_dotenv('.env')

_ = os.getenv('TEAM_API_KEY')
gemini2_id = os.getenv('GEMINI2_FLASH_ID')

from aixplain.factories import ModelFactory

"""
model_list = ModelFactory.list(
    function=Function.TEXT_GENERATION, page_size=50)["results"] #type: ignore
for model in model_list:
    print(model.id, model.name, model.supplier)

"""

class ModelAccessException(Exception):
    def __init__(self, message, model_id):
        super().__init__(message)
        self.model_id = model_id

def query_model(prompt: str, model_id: str) -> str :
    if model_id:
        gemini2 = ModelFactory.get(model_id=model_id)
        result = gemini2.run(data=prompt, parameters={"max_tokens": 4096})
        return result.data
    else:
        raise ModelAccessException("Cannot access model", model_id)

def typing_effect(text: str, min_delay=0.02, max_delay=0.1) -> None:
    for char in text:
        sys.stdout.write(char)
        sys.stdout.flush()
        time.sleep(random.uniform(min_delay, max_delay))
    print()

prompt: str | None = None
try:
    with open('./prompts/005_prompt.txt', 'r') as file:
        prompt = file.read()
    if prompt and gemini2_id:
        gemini2_output = query_model(prompt, gemini2_id)
        wrapped_gemini2_output = textwrap.fill(gemini2_output, width=100)
        # print(gemini_output)

        os.system('clear')
        print(f'\033[34m{'-' * 100}\033[0m')
        typing_effect(wrapped_gemini2_output, max_delay=0.05)
        print(f'\033[34m{'-' * 100}\033[0m')
except ModelAccessException as e:
    print(f'[ERROR]: {e}')
    print(f'model_id: {e.model_id}')
